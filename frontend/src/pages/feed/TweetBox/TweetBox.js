...
function TweetBox (){
    ...
    const handleTweet=(e)=>{
        ...
        if(user.providerData[0].providerId === 'password'){
          fetch(`https://twitter-dd3q.onrender.com/loggedInUser?email=${email}`)
          .then(res=>res.json())
          .then(data=>{
              setName(data[0]?.name)
      ...
      <div className="tweetBox"  >
        <form onSubmit={handleTweet}>
          <div className="tweetBox__input">
            ...
            <input type='text' placeholder={t("What's Happening?!")} 
          ...
          <div className="imageIcon_tweetButton">
            ...
            <Button className='tweetBox__tweetButton'type='submit'>{t('post')} </Button>
          </div>
        </form>
      </div>
