...
useEffect(()=>{
 fetch(`https://twitter-dd3q.onrender.com/userPost?email=${user?.email}`)
 .then(res=>res.json())
 .then(data=>{
  setPosts(data)
 })
...
const handleUploadCoverImage=(e)=>{
    ...
    .then(res=>{
      ...
      setIsLoading(false);
      if(url){
       axios.patch(`https://twitter-dd3q.onrender.com/userUpdates/${user?.email}`,userCoverImage)
      }
    })
...
const handleUploadProfileImage=(e)=>{
    ...
    .then(res=>{
      ...
      setIsLoading(false);
      if(url){
        axios.patch(`https://twitter-dd3q.onrender.com/userUpdates/${user?.email}`, userProfileImage)
       }
    })
}
    return (
